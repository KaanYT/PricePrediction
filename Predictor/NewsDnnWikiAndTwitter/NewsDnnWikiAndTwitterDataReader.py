...
class NewsDnnWikiAndTwitterDataReader(object):
    ...
    def fetch_train_data(self):
        ...
        self.__train_cursor = self.__train_cursor.sort(NewsDnnWikiAndTwitterDataReader.get_sort_list(self.configs['train_query_sort']))
    ...
    def get_train_data(self):
        ...
        for row in self.__train_cursor:
            embedded_article = self.word_embedding.get_weight_matrix_with_wiki_tweet(row["article"],
                                                                                     row["relatedness"],
                                                                                     row["tweet_percentage"])
            if self.configs["wiki"]["skip"] and row["relatedness"] < self.configs["wiki"]["value"]:
                continue
            if len(embedded_article) < NewsDnnWikiAndTwitterDataReader.ArticleMinSize:
                continue
            ...
            self.y.append(NewsDnnWikiAndTwitterDataReader.get_classification(row[price_start],
                                                                   row[price_end]))
    ...
    def fetch_test_data(self):
        ...
        self.__test_cursor = self.__test_cursor.sort(NewsDnnWikiAndTwitterDataReader.get_sort_list(self.configs['test_query_sort']))
    ...
    def get_test_data(self):
        ...
        for row in self.__test_cursor:
            ...
            if shape[0] != self.sequence_length:
                continue
            self.x.append(self.pad_embedded_article(embedded_article))
            self.y.append(NewsDnnWikiAndTwitterDataReader.get_classification(row["price_before"],
                                                                             row["price_after_day"]))
            batch_count = batch_count + 1
            if batch_count % self.batch_size == 0:
    ...
    def fetch_validate_data(self):
        ...
        self.__validate_cursor = self.__validate_cursor.sort(NewsDnnWikiAndTwitterDataReader.get_sort_list(self.configs['validate_query_sort']))
